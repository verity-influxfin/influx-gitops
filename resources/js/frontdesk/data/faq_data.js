export default {
    "college": [
        {
            "title": "誰可以使用學生貸？",
            "content": "年滿20歲之大學在學生。"
        },
        {
            "title": "持證自拍不清晰？",
            "content": "1.持證自拍時請留意證件與人臉都需清晰可辨識。<br>2.證件不可擋到臉部，手也不可擋到證件。<br>3.請盡量於背景單純明亮的拍攝，若有配戴眼鏡建議將眼鏡拿下。"
        },
        {
            "title": "SIP是什麼？",
            "content": "SIP為登入學校線上系統使用的資訊。<br>如您的校方信箱無法順利收信，SIP資訊能協助您快速完成學生認證，請填寫正確資訊。"
        },
        {
            "title": "學生證遺失怎麼辦？",
            "content": "可使用校方開立並有校方戳章的在學證明代替。"
        },
        {
            "title": "非中華民國國民或境外學校可以申請嗎？",
            "content": "目前尚未開放非中華民國國民及境外學校學生申請。"
        }
    ],
    "freshgraduate": [
        {
            "title": "聯徵資料如何取得完成驗證？",
            "content": "申請聯徵有下列三種方式：<br>1.使用自然人憑證線上申請<br>2.聯徵中心現場申請紙本聯徵報告<br>3.郵局臨櫃申請"
        },
        {
            "title": "工作驗證需甚麼資料？",
            "content": "工作驗證必備資料如下<br>1.近一個月申請勞保異動明細<br>勞保異動明細可親洽勞保局申請，也可使用自然人憑證下載電子檔<br>2.近三個月薪資證明<br>薪資證明可提供存摺封面與近三個月薪資轉入的存摺內頁<br>或是網銀包含個人姓名與帳號的畫面,以及近三個月薪資轉入的畫面<br>3.其他選填資料"
        },
        {
            "title": "畢業證書遺失怎麼驗證學歷？",
            "content": "1.拍攝畢業證書正本<br>2.拍攝在學期間成績單<br>3.提供在學期間SIP資訊"
        },
        {
            "title": "上班族貸的申請條件是？",
            "content": "20-35歲完成工作認證及聯徵認證，均可申請。"
        },
        {
            "title": "借款手續費怎麼算？",
            "content": "平台僅收取本金的4% 未完成最高學歷認證為5%，最低為500元，作為平台服務費。"
        }
    ],
    "engineer": [
        {
            "title": "工程師優惠專案的申請資格？",
            "content": "學生 : 年滿20歲資訊軟體相關系所的大學以上在校生。<br>上班族：年滿20以上大學以上，需資訊軟體相關系所畢業上班族，並從事資訊科技類相關的從業人員。"
        },
        {
            "title": "工程師們有甚麼好康？",
            "content": "普匯針對擁有專業技能的Techi們提供更優惠的專案，借款額度高達20萬、利率減免優惠5%起，且超快速半小時審核過件。"
        },
        {
            "title": "工程師優惠申請金額有上限嗎？",
            "content": "有的，學生申請上限為12萬，上班族為20萬。"
        },
        {
            "title": "借款手續費怎麼算？",
            "content": "手續費為學生3% 上班族4%，最低為500元，作為平台服務費。"
        }
    ],
    "invest": [
        {
            "title": "投資標的逾期該怎麼辦？",
            "content": "若您的債權逾期了，普匯平台會啟動催收機制，協助解決催理困擾；建議同步嘗試使用債權/折價轉讓功能，將原有債權轉讓出售。"
        },
        {
            "title": "債權投資的利率是公開的嗎？",
            "content": "普匯債權投資的利率完全公開透明，於投資時即可觀看每項產品的利率,還款期數,金額與收益等資訊,再依個人需求,自由選擇您要的標的。"
        },
        {
            "title": "我所投資的對象是誰？",
            "content": "平台的借款人都是經由AI風險控管的案件，對象為年滿20歲以上的公民，包括： 學生、上班族 、企業主。您可以於投標前選擇您想投資的對象。"
        }
    ],
    "transfer": [
        {
            "title": "什麼是債權轉讓？",
            "content": "當您需將手上債權變現時，可使用債權轉讓功能，將手上債權拋轉上架 供其他投資人認購，提早回流資金，自由運用。"
        },
        {
            "title": "債權已經逾期，還能夠轉讓嗎？",
            "content": "可以的，無論是正常還款中，或是逾期的債權，都可以做轉讓喔。可選擇折價轉讓，快速資金回流。"
        },
        {
            "title": "債權轉讓可以轉讓給指定的投資人嗎？如何同時轉讓多筆債權？",
            "content": "可以的，您可以選擇批次轉讓之打包功能，並設定密碼，提供予特定投資人進行轉讓。"
        }
    ]
}