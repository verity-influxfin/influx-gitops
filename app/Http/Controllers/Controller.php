...
class Controller extends BaseController
{
    ...
    public function getTransferCase(Request $request){
        ...
        $case_response = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/transfer_list?' . $params . '"');
        ...
        if(isset($input['product_id']) && isset($input['status'])){
            if(in_array($input['product_id'],[0,1,3]) || in_array($input['status'],[3,10])){
                ...
                if($input['product_id'] == 0){
                    $input['product_id'] = 1;
                    $params = http_build_query($input);
                    $case_response_1 = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/list?' . $params . '"');

                    $input['product_id'] = 3;
                    $params = http_build_query($input);
                    $case_response_3 = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/list?' . $params . '"');
                    $case_response = '';
                    ...
                    $params = http_build_query($input);
                    $case_response = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/list?' . $params . '"');
                }
                if(!empty($case_response)){
                    $case_data = json_decode($case_response,true);
    ...
    public function getOption(Request $request)
    {
        ...
        if(isset($input['data']) && !empty($input['data'])){
            if(isset($option_config[$input['data']])){
                ...
                $response = shell_exec('curl -k --location --request GET "' . $this->apiGetway . $api_url .'"');
            }
    ...
    public function getBorrowReport(Request $request)
    {
        ...
        if(isset($input['identity']) && !empty($input ['identity'])){
            ...
            if($input ['identity'] == 1){
                $total_point += 1150;
                $school_points = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/credit_school"');
                $department_points = shell_exec('curl -k --location --request GET "' . $this->apiGetway . 'website/credit_department"');
