...
while($row = mysqli_fetch_assoc($result)){
	...
	if($row['Tables_in_fucoin_xunibi']){
		$table  = $row['Tables_in_fucoin_xunibi'];
			...
			if($data && isset($data['id'])){
				...
				foreach($data as $key => $value){
					if(strpos($value,"'")==false)
						$set[] = "`".$key."`='".$go->c2t($value)."'";
				}
				$sql = "UPDATE `$table` SET ".implode(",",$set)." WHERE id=".$data['id'];
...
/*$dir 	= "/var/www/html/fucoin/web/Application/";
$list 	= get_dir_list($dir);
if(!empty($list)){
	foreach($list as $key =>$value){
		if(strpos($value,".php")!==false || strpos($value,".html")!==false){
			$array 		= file($value);
			$content 	= "";
    ...
    if(is_dir($dir)){
        ...
        while (($file = readdir($dh)) !== false) {
            if (is_dir($file) && basename($file)!='.' && basename($file)!='..'){
                $rs = get_dir_list($file);
				$list = array_merge($rs,$list);
            }else if($file != "." && $file != ".."){
				$list[] =  getcwd()."/$file";
            }
        }
