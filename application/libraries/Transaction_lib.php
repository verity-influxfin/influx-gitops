    ...
    function withdraw_success($withdraw_id)
    {
        ...
        if ($withdraw_id) {
            ...
            $transaction_commit = function () {
                if (
                    $this->CI->withdraw_model->trans_status() === FALSE ||
        ...
        if ($mrs && $rs) {
            ...
            try {
                ...
                foreach ($transfers as $t => $transfer) {
                    ...
                    $transaction_list = $this->CI->transaction_model->get_many_by(
                        array(
                    ...
                    $transfer_investments = $this->CI->transfer_investment_model->get_by(
                        array(
    ...
    function subloan_success($target_id, $admin_id = 0)
    {
        ...
        if ($target_id) {
            ...
            if ($target && $target->status == 4 && $target->loan_status == 2 && $this->CI->target_lib->is_sub_loan($target->target_no) === TRUE) {
                ...
                $subloan = $this->CI->subloan_model->get_by(
                    array(
                        'status' => 2,
