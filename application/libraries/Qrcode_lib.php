...
class Qrcode_lib
{
    ...
    public function get_promoted_reward_info(array $where, string $start_date = '', string $end_date = '', int $limit = 0, int $offset = 0, bool $filter_delayed = FALSE, bool $merge_subcode = TRUE): array
    {
        ...
        $no_identity_list = [];
        $have_identity_list = [];
        foreach ($subcode_list as $subcode)
        {
            // 下線沒有經過實名認證時不予計算業績
            $identity = $this->CI->qrcode_lib->get_user_identity($subcode['registered_id']);
            if ( ! $identity)
            {
                $no_identity_list[] = $subcode['user_qrcode_id'];
            }
            else
            {
                $have_identity_list[] = $subcode['user_qrcode_id'];
            }
