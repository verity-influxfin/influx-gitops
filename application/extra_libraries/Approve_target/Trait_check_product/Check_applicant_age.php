...
trait Check_applicant_age
{
    ...
    {
        if ($this->CI->product_lib->need_chk_allow_age($target['product_id']) === TRUE)
        {
            ...
            if ($this->CI->product_lib->is_age_available($age, $target['product_id'], $target['sub_product_id']) === FALSE)
            {
                return FALSE;
            }
        }
