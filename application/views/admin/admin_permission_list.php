...
<div id="page-wrapper">
    <div class="row">
        <h1 class="page-header">人員權限設定</h1>
    </div>
    <div class="panel panel-default">
        ...
        <div class="new-role m-4">
            <a class="btn btn-primary" href="<?= admin_url('admin/admin_permission_add') ?>" target="">新增人員權限</a>
        </div>
        <div class="table-row">
...
<script>
    $(document).ready(() => {
        ...
        $('.search-btn button').on('click', function () {
            let name = $('#search').val();
            top.location = `./admin_permission_list?name=${name}`;
        });
    });

    function insertDataRow({table, account, name, part, group, role, id}) {
        const origin = window.location.origin
        const button = `<button class="btn btn-default" onClick="window.open('${origin}/admin/admin/admin_permission_edit?id=${id}','_self')">Edit</button>`
        table.row.add([account, name, part, group, role, button]).draw()
    }

    function get_dataRow(table) {
        $.ajax({
            url: 'get_admin_permission_list' + window.location.search,
            type: 'GET',
            dataType: 'JSON',
