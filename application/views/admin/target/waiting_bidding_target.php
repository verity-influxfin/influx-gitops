                                ...
                                <table class="table table-striped table-bordered table-hover" width="100%" <? if(!isset($input['target_id'])){ ?>id="dataTables-tables<? echo $typeKey; }?>">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>會員 ID</th>
                                            <th>信用等級</th>
                                            <th><? echo $typeKey == 'personal' ? '公司/學校' : '公司名稱' ?></th>
									...
									?>
                                        <tr class="<?=$count%2==0?"odd":"even"; ?>">
                                            ...
                                            <td><?=isset($value->invested) && $typeKey != 'judicialPersonFormBank'? $value->invested :($status_list[$value->status]) ?></td>
											...
											<td><a class="btn btn-primary btn-info" href="<? echo $typeKey != 'judicialPersonFormBank' ? admin_url('creditmanagementtable/waiting_bidding_report?target_id='.$value->id.'&table_type=management'):admin_url('target/waiting_reinspection'."?target_id=".$value->id)?>" target="_blank" >查看</a></td>
