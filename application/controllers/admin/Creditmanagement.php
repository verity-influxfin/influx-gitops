...
class Creditmanagement extends MY_Admin_Controller
{
    ...
    public function report(){
        ...
        if(empty($target_id) || empty($type)){
            print_r('parameter can not empty');
            return;
        }
    ...
    public function get_data(){
        ...
        $reason  = isset(json_decode($target_info->reason,true)['reason_description']) ?  json_decode($target_info->reason,true)['reason_description'] : $target_info->reason;
        $target_info = $this->target_model->order_by('id','ASC')->get_by(['user_id'=>$target_info->user_id]);

        $firstApplyDate = date('Y-m-d',$target_info->created_at);
        $response = [
            'firstApplyDate' => $firstApplyDate,
            'reason' => $reason
        ];

        $user_certification_info = $this->user_certification_model->order_by('id','DESC')->get_by(['user_id'=>$target_info->user_id,'certification_id'=>1,'status'=>1]);
        if(!empty($user_certification_info)){
            $user_certification_info = json_decode($user_certification_info->content,true);
            $response['name'] = $user_certification_info['name'];
            $response['id_card_number'] = $user_certification_info['id_number'];
            $response['birthday'] = $user_certification_info['birthday'];
            $response['address'] = $user_certification_info['address'];
        }

        // $this->credit_management->get_data();
