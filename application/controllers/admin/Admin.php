...
class Admin extends MY_Admin_Controller {
    ...
    //角色名稱
    public $position_list = [1 => '執行長', 2 => '主管', 3 => '經辦'];
    ...
    public function role_list_setting()
    {
        $this->load->view('admin/_header');
        $this->load->view('admin/_title', $this->menu);
        $this->load->view('admin/role_list_setting');
        $this->load->view('admin/_footer');
    }
    ...
    public function role_list_edit()
    {
        ...
        if (empty($post)) // 瀏覽表單
        {
            ...
            if ( ! $id)
            { // query string沒有帶id
                alert('ERROR , id is not exist', admin_url('Admin/role_list_setting'));
            }
        ...
        else // 儲存表單
        {
            ...
            $check_result = $this->check_form_data(
                $post,
                ['division', 'department', 'position', 'permission']
            );
            ...
            {
                alert('更新成功', admin_url('Admin/role_list_setting'));
            }
            else
            {
                alert('更新失敗，請洽工程師', admin_url('Admin/role_list_setting'));
            }
        }
    }
    ...
    public function role_list_add()
    {
        ...
        else // 儲存表單
        {
            ...
            {
                alert('新增成功', admin_url('Admin/role_list_setting'));
            }
            else
            {
                alert('新增失敗，請洽工程師', admin_url('Admin/role_list_setting'));
            }
        }
    }
    ...
    // 人員管理-取列表資料
    public function role_management_get()
    {
        $get = $this->input->get(NULL, TRUE);
        $where = [];
    ...
    public function role_management_edit()
    {
        ...
        if (empty($post)) // 瀏覽表單
        {
            ...
            if ( ! $id)
            { // query string沒有帶id
                alert('ERROR , id is not exist', admin_url('Admin/role_management'));
            }
        ...
        else // 儲存表單
        {
            ...
            {
                alert('更新成功', admin_url('Admin/role_management'));
            }
            else
            {
                alert('更新失敗，請洽工程師', admin_url('Admin/role_management'));
            }
        }
    }

    // 人員管理-新增
    public function role_management_add()
    {
        $post = $this->input->post(NULL, TRUE);
        ...
        else // 儲存表單
        {
            ...
            {
                alert('新增成功', admin_url('Admin/role_management'));
            }
            else
            {
                alert('新增失敗，請洽工程師', admin_url('Admin/role_management'));
            }
        }
    }
    ...
    public function role_list_review()
    {
        $this->load->view('admin/_header');
        $this->load->view('admin/_title', $this->menu);
        $this->load->view('admin/role_list_review');
        $this->load->view('admin/_footer');
    }

    // 權限審查-取列表資料
    public function role_list_review_get()
    {
        $get = $this->input->get(NULL, TRUE);
        $where = [];
        if (isset($get['name']) && ! empty($get['name']))
    ...
    // 權限審查-細節
    public function role_review_edit()
    {
        $get = $this->input->get(NULL, TRUE);
        ...
        if ( ! $id)
        { // query string沒有帶id
            alert('ERROR , id is not exist', admin_url('Admin/role_list_review'));
        }
     ...
     * @return array
     */
    public function check_form_data(array $post, array $fields)
    {
        $result = TRUE;
        $data = [];
        $msg = '';
    ...
    // 更新權限審查啟用狀態(admins.permission_status)
