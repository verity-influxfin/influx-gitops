	...
	public function credit_post()
	{
		...
		if (!empty($list)) {
			foreach ($list as $s3_url) {
				...
				$filename=$this->s3_lib->public_get_filename($s3_url,$bucket = 'influx-mailbox');
				$file_content  =  file_get_contents('s3://'.$bucket.'/'.$filename);
				...
				if (empty($user_info)||(($re_investigation_mail === false)&&($re_job_mail === false))) {
					$this->s3_lib->unknown_mail($s3_url,$bucket = 'influx-mailbox');
					$this->s3_lib->public_delete_s3object($s3_url,$bucket = 'influx-mailbox');
					return null;
				}
